...
global {
	...
	// 1. NUEVO: Parámetro de texto para elegir el mapa
    // Ponemos como valor por defecto el que ya tienes
    string nombre_mapa <- "mapaCarolina.shp" among: ["mapaCarolina.shp", "red_vial_la_carolina.shp", "mapaQuicentroSur.shp"];

    // 2. Modificamos la carga para que use la variable dinámica
	...
	create PuntoControl with: [
		...
		nombre_acceso::"PC_" + string(length(puntos_control_usuario)),
		location::punto_pendiente,
		tarifa_vigente::0.0,
		modo_peaje_activo::true
	];

	write "Punto de control agregado en: X="
		+ string(punto_pendiente.x) + " Y=" + string(punto_pendiente.y);

	punto_pendiente <- nil;
}

// ── INYECCIÓN DINÁMICA DE TRÁFICO EN HORA PICO ───────────────────────────
	// Crea flujos adicionales de vehículos simulando los ingresos masivos a La Carolina
	reflex inyectar_trafico_hora_pico when: es_hora_pico and (dia_semana >= 1 and dia_semana <= 5) {
		
		// Cada 3 ciclos (30 segundos de simulación) se genera una nueva oleada
		if (cycle mod 3 = 0) {
			list<road> pool_vias <- empty(roads_conectadas) ? list(road) : roads_conectadas;
			
			// Generamos un grupo pequeño de vehículos mixtos en puntos aleatorios de la red vial
			create ConductorBDI number: 2 {
				...
				// Forzamos a que recalculen su comportamiento BDI de inmediato al entrar
				decision_tomada <- false; 
			}
			
			// Incrementamos también un par de buses de transporte público exonerados
			if (cycle mod 12 = 0) {
	...
	// Coordenadas reales en UTM 17S (EPSG:32717). GAMA NORMALIZA el shapefile al
	// cargarlo, así que el UTM crudo no calza: se convierte con to_GAMA_CRS(...,
	// CRS_DATOS), que aplica el mismo desplazamiento que GAMA usó en la red. Por eso
	// zona_peaje y las posiciones se asignan en init (tras cargar el shapefile).
	...
	init {
		...
		// Coordenadas UTM reales convertidas con to_GAMA_CRS para calzar con la red.
		// list<point> puntos_control_usuario <- [];
		
		// ── Estaciones Metro ─────────────────────────────────────────────────
		// Estaciones reales (UTM 17S, Quito) convertidas con to_GAMA_CRS. Igualadas
		// a E0: Iñaquito (norte) y La Carolina (sur). Antes FIX-6 usaba "El Labrador"
		// con coords placeholder; se renombró a La Carolina para coincidir con E0.
		create EstacionMetro with: [nombre::"Iñaquito",
		    location::(to_GAMA_CRS({780119.0, 9980458.0}, CRS_DATOS)).location];
		create EstacionMetro with: [nombre::"La Carolina",
		    location::(to_GAMA_CRS({779831.0, 9978891.0}, CRS_DATOS)).location];
		...
		// §3.2 fix: los contadores de los charts arrancan en 0. Antes se inicializaban
		// con el tamaño de la flota, lo que inflaba el sector "Ruta directa" del pastel.
		// Ahora solo crecen con decisiones BDI reales (en `decidir`).
		chart_ruta_directa <- 0;
		chart_reroutean    <- 0;
		chart_metro        <- 0;
		chart_restringidos <- 0;
		...
		if (nombre_mapa contains "Sur") {
			step <- 2 #s; // Baja el paso del tiempo de 10 segundos a 2 segundos por ciclo
		}
	}
	...
	reflex exportar_metricas when: (cycle mod INTERVALO_LOG = 0) and (cycle > 0) {
		...
		// BUG-2 fix: cuota modal como SNAPSHOT de la intención actual de la flota
		// (no contadores de eventos). Mantiene coherencia con E0 y cuenta bien a los
		// conductores que no deliberaron como RUTA_DIRECTA.
...
float  tarifa_base <- 0.0 update: (es_hora_pico ? TARIFA_PICO_AUTO : TARIFA_VALLE);
	...
	// Compuerta de acceso sobre la frontera de la zona. Pórtico que se eleva y
	// enrojece cuando cobra; gris/bajo cuando la tarifa está en $0.
	aspect default {
	bool activo <- modo_peaje_activo and tarifa_vigente > 0;
	rgb  c      <- activo ? rgb(220, 40, 40) : rgb(110, 120, 130);

	draw box(36, 36, (activo ? 90 : 40)) 
		at: location color: rgb(c, 0.92) border: #white;

	draw "C" + id_control 
		at: location + {0, 0, (activo ? 100 : 50)}
		color: #white font: font("Arial", 13, #bold);


	if activo {
		draw "$" + round(tarifa_vigente * 100) / 100.0
			at: location + {0, 80, 105}
			color: #yellow font: font("Arial", 11, #bold);
	}
}
}
...
species ConductorBDI skills: [moving] {
    ...
    if (nombre_mapa contains "Sur") {
			speed <- speed * 0.08; // Si van demasiado rápido todavía, baja este 0.15 a 0.08
		}
	}
...
if (restringido_placa and minuto_actual >= RESTRICCION_INICIO and minuto_actual <= RESTRICCION_FIN) {
			...
			// FIX-1 BLINDADO: Verificar que la lista no esté vacía Y que exista una estación válida cerca
			if (intencion = "METRO" and not empty(EstacionMetro) and (EstacionMetro closest_to self != nil)) {
				...
				// FALLBACK: Si quería usar METRO pero no hay estaciones en este mapa (ej. mapa Sur), cambia a REROUTEAR
				intencion <- "REROUTEAR";
				
				// Contadores para REROUTEAR
		...
		if (ud >= ur and ud >= um) {
			...
			// FIX-2 BLINDADO: Verificar que existan estaciones y que no devuelva nil antes de hacer el ask
			if (not empty(EstacionMetro) and (EstacionMetro closest_to self != nil)) {
				intencion <- "METRO";
				ask (EstacionMetro closest_to self) {
					if not saturada { pasajeros_espera <- pasajeros_espera + 1; }
				}
				
				// Contadores desagregados para METRO por elección
				if      (nse = "ALTO")  { count_metro_alto  <- count_metro_alto  + 1; }
				else if (nse = "MEDIO") { count_metro_medio <- count_metro_medio + 1; }
				else                    { count_metro_bajo  <- count_metro_bajo  + 1; }
			} else {
				// FALLBACK: Si prefería Metro pero no hay estaciones en este mapa (ej. mapa Sur), se desvía
				intencion <- "REROUTEAR";
				if      (nse = "ALTO")  { count_rerouta_alto  <- count_rerouta_alto  + 1; }
				else if (nse = "MEDIO") { count_rerouta_medio <- count_rerouta_medio + 1; }
				else                    { count_rerouta_bajo  <- count_rerouta_bajo  + 1; }
	...
	// Dentro de la zona de cobro: color pleno + borde blanco (foco). Fuera:
	// atenuado, para que la frontera del peaje resalte y se distinga el
	// tráfico de paso del periférico.
...
experiment "EB — Peaje Franja Horaria" type: gui autorun: false {
	...
	output synchronized: true {
		...
		event #mouse_down action: marcar_punto_control;
			...
			// Un solo acento (naranja = marca); color semántico solo en PICO, estado
			// de zona y tarifa activa. Divisores + tamaño marcan la jerarquía.
			overlay position: {18 #px, 18 #px} size: {250 #px, 392 #px}
			        background: rgb(15, 17, 22) border: rgb(70, 75, 85) rounded: true {
				int hh <- int(minuto_actual / 60);
				int mm_ <- minuto_actual mod 60;
				string reloj <- (hh < 10 ? "0" : "") + hh + ":" + (mm_ < 10 ? "0" : "") + mm_;
				...
				draw "Escenario EB · Peaje por franja horaria" at: {14 #px, 33 #px} anchor: #top_left
				...
				// Fila A: velocidad + estado de zona
				...
				draw "ZONA" at: {140 #px, 102 #px} anchor: #top_left
				     color: rgb(130, 135, 145) font: font("Arial", 8, #bold);
				draw (cobrando ? "COBRO ACTIVO" : "LIBRE") at: {140 #px, 115 #px} anchor: #top_left
				     color: (cobrando ? rgb(230, 70, 70) : #limegreen) font: font("Arial", 13, #bold);

				// Fila B: tarifa (roja solo si cobra) + recaudación
				draw "TARIFA" at: {14 #px, 140 #px} anchor: #top_left
				     color: rgb(130, 135, 145) font: font("Arial", 8, #bold);
				draw ("$" + (round(t_actual * 100) / 100.0)) at: {14 #px, 153 #px} anchor: #top_left
				     color: (cobrando ? rgb(230, 70, 70) : #white) font: font("Arial", 13, #bold);
				draw "RECAUDADO" at: {140 #px, 140 #px} anchor: #top_left
				     color: rgb(130, 135, 145) font: font("Arial", 8, #bold);
				draw ("$" + (round(recaudacion_acum * 100) / 100.0)) at: {140 #px, 153 #px} anchor: #top_left
				     color: #white font: font("Arial", 13, #bold);
				draw rectangle(222 #px, 1 #px) at: {125 #px, 180 #px} color: rgb(55, 60, 70);
			...
			// ── Zona de cobro: relleno + frontera que reacciona al cobro ──────────
			// Hace visible dónde empieza/termina la zona (el objeto central del modelo).
			// El polígono se tiñe de rojo cuando hay tarifa vigente y de azul neutro
			// cuando está libre; los conductores dentro van a color pleno (ver aspect).
			graphics "Zona de cobro" {
				bool cobrando_z <- not empty(PuntoControl) and first(PuntoControl).tarifa_vigente > 0.0;
				rgb  base       <- cobrando_z ? rgb(220, 50, 40) : rgb(46, 109, 164);
				draw zona_peaje color: rgb(base, cobrando_z ? 0.16 : 0.10) border: rgb(base, 0.95);
				// Etiqueta discreta sobre el borde inferior (no flota grande en el centro).
				draw (cobrando_z ? "ZONA DE COBRO · ACTIVA" : "ZONA DE COBRO")
				     at: lbl_zona anchor: #left_center
				     color: (cobrando_z ? rgb(255, 130, 120) : rgb(140, 175, 215))
			...
			// NOTA: los displays "Reparto modal en el tiempo" y "Recaudación" se
			// retiraron — su evolución se analiza en el pipeline Python (columnas
			// pct_* y recaudacion del CSV). En la vista viva queda el pastel
			// "Decisiones BDI" como instantánea.

		// ── Decisiones BDI ────────────────────────────────────────────────────
		display "Decisiones BDI — EB" {
			// Dona con etiquetas sobre las porciones → el % de cada decisión se lee
			// directamente. Snapshot de la intención actual de la flota (= CSV).
		...
		// NOTA: el display "Velocidad y Tarifa" se retiró — velocidad casi plana
		// (~30 km/h) y tarifa en escalón aportan poco como serie viva; ambos
		// valores siguen visibles en el HUD del mapa y se analizan en el pipeline.

		// ── Equidad NSE ───────────────────────────────────────────────────────
		// Una barra apilada por nivel socioeconómico (Alto/Medio/Bajo), compuesta
		// por las 3 decisiones. Antes eran 9 series con nombres largos solapados e
		// ilegibles en el eje X; ahora hay solo 3 etiquetas claras y un legend de 3
		// colores. Se lee de un vistazo CÓMO decide cada grupo frente al peaje
		// (p. ej. si el NSE Bajo se va al Metro y el Alto paga la ruta directa).
		display "Equidad NSE — EB" {
			chart "Equidad: decisión por nivel NSE (acumulado)" type: histogram style: stack
			      background: rgb(25, 25, 25) color: #white
