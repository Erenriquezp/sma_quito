...
> *Insertar `analysis/figures/` o captura QGIS: polígono georreferenciado del Parque La Carolina con los puntos de control C1–C5 y las estaciones intermodales del Metro.*
...
| SUV | 45 (15 %) | 1.5 | 2.00 | $wtp$ escalado ×1.3. |
...
$$\tau_k(t) = \begin{cases} \$2.00 & k \in \{\text{Auto, SUV}\},\ t \in \text{pico} \\ \$3.00 & k = \text{Carga},\ t \in \text{pico} \\ \$0.00 & k \in \{\text{Moto, Bus}\} \ \lor\ t \notin \text{pico} \end{cases} \tag{2}$$

con franjas pico de 07:00–10:00 y 17:00–20:00. La tarifa que cada agente realmente paga (**tarifa efectiva**) se calculó como un ratio respecto a la tarifa del Auto, según la ecuación (3):

$$\tau_{ef} = \tau_{\text{Auto}} \cdot \frac{\tau_k}{\tau_{\text{Auto}}}, \quad \tau_{ef}=0 \text{ si el tipo está exonerado} \tag{3}$$

Sobre esta señal base operó un **agente gestor (GestorAMT)** con arquitectura BDI deliberativa que, cada 30 ciclos, ajustó dinámicamente la tarifa vigente en el rango $[\$0.50,\ \$3.00]$ en pasos de $\$0.25$, seleccionando entre las intenciones MANTENER, SUBIR, BAJAR o SUSPENDER en función de la densidad del polígono, la velocidad media y la saturación del Metro, y difundiéndola a los puntos de control mediante un patrón FIPA.
...
> **[FIGURA 2 — Entorno GAMA]**
> *Insertar captura del entorno gráfico de GAMA Platform durante la ejecución interactiva de la simulación de tráfico continuo.*
...
|---|---:|---:|---:|:---:|
| Flujo interno — total (veh/h) | 2134.31 | 2141.62 | +0.34 | ✗ |
| **Flujo interno — hora pico (veh/h)** | 2008.15 | 1869.38 | **−6.91** | ✓ |
| Desplazamiento periférico (veh/h) | 129.69 | 149.56 | +15.33 | ✗ |
| Velocidad media — total (km/h) | 33.87 | 33.77 | −0.32 | ✗ |
| **Velocidad media — hora pico (km/h)** | 33.89 | 34.31 | **+1.25** | ✓ |
| Modal shift → Metro — total (%) | 4.52 | 3.12 | −31.08 | ✗ |
| **Modal shift → Metro — hora pico (%)** | 5.24 | 7.65 | **+46.04** | ✓ |
| Δ Gini modal — hora pico (0–1) | 0.14 | 0.19 | +40.34 | ✗ |
| Recaudación estimada (USD/h) | 0.00 | 542.62 | — | — |

La respuesta dinámica del volumen interno ante la activación de la ventana tarifaria se grafica en la Fig. 3, y la velocidad media comparada en la Fig. 4. La bifurcación de intenciones (modal shift) entre perfiles se visualiza en las Figs. 5 y 6.

> **[FIGURA 3 — Flujo vehicular]**
> *Insertar `analysis/figures/fig1_flujo_vehicular.pdf`: curvas temporales comparativas del volumen de vehículos activos dentro del perímetro de La Carolina (E0 vs EB).*

> **[FIGURA 4 — Velocidad media]**
> *Insertar `analysis/figures/fig2_velocidad_media.pdf`: evolución de la velocidad media en el polígono a lo largo del día.*

> **[FIGURA 5 — Decisiones BDI]**
> *Insertar `analysis/figures/fig3_decisiones_bdi.pdf`: distribución de intenciones (directa / reroutear / Metro).*

> **[FIGURA 6 — Modal shift]**
> *Insertar `analysis/figures/fig4_modal_shift.pdf`: transferencia modal hacia el Metro por franja horaria.*

> **[FIGURA 7 — Benchmark Londres]**
> *Insertar `analysis/figures/fig5_benchmark_londres.pdf`: comparación de indicadores del modelo EB frente a las metas del London Congestion Charge.*

> **[FIGURA 8 — Equidad y recaudación]**
> *Insertar `analysis/figures/fig6_equidad_recaudacion.pdf`: índice de Gini modal y recaudación acumulada.*

Durante la hora pico, el peaje redujo el flujo interno de 2008.15 a 1869.38 veh/h (−6.91 %) y elevó ligeramente la velocidad media (+1.25 %). La transferencia modal al Metro en pico creció de 5.24 % a 7.65 % (+46.04 %). En contraste, el desplazamiento periférico aumentó un 15.33 % y el índice de Gini modal en pico pasó de 0.14 a 0.19 (+40.34 %), indicando mayor inequidad de acceso modal. El escenario EB acumuló una recaudación de 8 682 USD (~542.62 USD/h efectivos en pico). El GestorAMT mantuvo la tarifa base de $2.00 durante las franjas de cobro (intención MANTENER predominante), sin alcanzar las condiciones de densidad y velocidad crítica que habrían disparado un escalamiento, lo que evidencia que el margen dinámico disponible no llegó a activarse en este escenario. Las pruebas t de Student arrojaron significancia únicamente para la transferencia modal (pct_metro: p = 0.013), mientras que el flujo (p = 0.946) y la velocidad (p = 0.638) no resultaron significativos. La Tabla 2 contrasta estos hallazgos con las metas del benchmark.

**Tabla 2.** Validación contrafactual frente al London Congestion Charge.

| Indicador | Londres 2003 | Meta Quito | Modelo EB | ¿Cumple? |
|---|---|---|---:|:---:|
| Reducción vehicular (hora pico) | −27 % | ≥ −20 % | −6.9 % | ✗ |
| Mejora de velocidad (hora pico) | +20 % | > 0 % | +1.2 % | ✓ |
| Modal shift a transporte público | ~12 % | > 5 % | ~7.6 % | ✓ |
| Tarifa pico | £5 ≈ $6.0 USD | $2.00–$3.00 propuesto | $2.00 Auto/SUV · $3.00 Carga · Moto/Bus exentos | — |
...
No obstante, la magnitud del efecto fue marcadamente inferior a la de los referentes internacionales. La evidencia muestra que un peaje urbano reduce el volumen de tráfico que ingresa a la zona central entre un 15 y un 27 % y disminuye la congestión en torno a un 30 % durante el primer año, incrementando las velocidades de circulación [5], [6], [7]. En el presente modelo, la reducción de flujo en hora pico (−6.9 %) no alcanzó el umbral de validación del 20 %, por lo que **la hipótesis principal no se confirmó**. Tres factores explican esta brecha. Primero, las tarifas evaluadas ($2.00 para autos y SUV, $3.00 para carga, con motos y buses exonerados) resultaron sustancialmente menores que la de Londres (£5 ≈ $6.0 USD en 2003); además, la exoneración de motos y buses —los modos más numerosos tras el auto— acotó la fracción de flota efectivamente disuadida, traduciéndose en una elasticidad de demanda débil. Segundo, se observó un efecto de desplazamiento periférico (+15 %), coherente con la literatura que advierte que parte del tráfico se reasigna a vías no cobradas en lugar de desaparecer [6]. Tercero, el peaje incrementó la inequidad modal (Gini pico +40 %), un resultado que contrasta con los beneficios progresivos documentados en zonas de bajos ingresos en Londres [6], y que en Quito se asocia a que los estratos de menor disposición a pagar son los forzados al cambio modal.
...
Como limitación metodológica, las velocidades medias simuladas (~34 km/h) excedieron el rango empírico de hora pico del DMQ (14–18 km/h), por lo que los valores absolutos deben leerse como tendencias relativas entre escenarios y no como predicciones puntuales. Asimismo, la colocación interactiva de los puntos de control en los modelos configurables limita por ahora la reproducibilidad en ejecución desatendida.

---

## 6. Conclusiones

El desarrollo del modelo multiagente integrando GAMA Platform y QGIS 3.x permitió evaluar con rigor el impacto de una política de tarificación vial en un entorno urbano andino. Respecto al **primer objetivo**, se concluyó que la ingesta de las redes viales vectoriales reales desde OpenStreetMap constituyó una plataforma geométrica de alta fidelidad, adecuada para el monitoreo de los flujos continuos. En relación con el **segundo objetivo**, los perfiles basados en la arquitectura BDI capturaron exitosamente las respuestas conductuales heterogéneas ante estímulos económicos disuasorios, superando las limitaciones de los enfoques promediados; el mecanismo deliberativo operó de forma consistente en ambos escenarios. Atendiendo al **tercer objetivo**, la comparación sistemática determinó que la zona de cobro disminuyó el tráfico de paso interno en hora pico (−6.9 %) e incrementó significativamente la demanda del Metro (+46 %, p = 0.013), pero **no alcanzó el umbral de reducción del 20 %** y generó externalidades de desplazamiento periférico (+15 %) e inequidad modal (+40 %).

En consecuencia, bajo los parámetros evaluados, el modelo no respaldó la hipótesis principal: el peaje propuesto produjo beneficios marginales con efectos colaterales relevantes. Este hallazgo, lejos de invalidar el instrumento, demostró su utilidad como herramienta predictiva de soporte a la decisión para el Plan Maestro de Movilidad del Municipio de Quito, capaz de anticipar que la medida —tal como está diseñada— rendiría poco. Como **trabajo futuro** se planteó: (i) recalibrar las tarifas diferenciadas hacia el rango $3–6 USD y revisar la exoneración de motos —que concentran parte del flujo disuasible— reejecutando EB; (ii) revisar la distribución de disposición a pagar y los umbrales de congestión por NSE; (iii) incorporar mecanismos de mitigación de equidad (exenciones por estrato socioeconómico); (iv) calibrar el modelo con aforos reales y externalidades ambientales; y (v) extender el análisis a los escenarios de peaje fijo 24 h y tarifa dinámica en tiempo real.
